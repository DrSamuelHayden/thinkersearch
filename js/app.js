...
function go_search(){
	...
	if(window.squery!=$("#q").val()){
		$("#res").css({"opacity":"1"});
$("#res").css({"background":"rgba(250,250,250,0.1)"});
		no_search = 1;
		set_query($("#q").val());
		if(tab=='web'){
			get_results(0);
...
function get_results(start){
	...
	if(no_fade==0){
		...
		$("#res_img").css({"background":"rgba(250,250,250,0.1)"});
		$("#res_yt").css({"opacity":"1"});
		$("#res_yt").css({"background":"rgba(250,250,250,0.1)"});
	}
	if(no_get==0){
		...
		success: function(response){
			if(response.results){
				...
				$.each(response.results, function(index, item){
					...
					$("#res").append("<div class='rbox clearfix' onclick='window.goto("+'"'+decodeURIComponent(item.url)+'"'+","+'"_blank"'+");'>"+wimg+"<img class='icon' src='https://plus.google.com/_/favicon?domain="+item.visibleUrl+"'/>&nbsp; <span class='rl'>"+item.title+"</span><br/><span class='g'>"+item.visibleUrl+"</span><br/>"+item.content+"<br/></div><br/>");
...
function checkScroll() {
 console.log($("#res").scrollTop());
    if(($("#res").scrollTop() >= $("#res").height() - 100) || ($("#res_img").scrollTop() >= $("#res_img").height() - 100) || ($("#res_yt").scrollTop() >= $("#res_yt").height() - 100)){
        if(tab=='web'){
			no_fade = 1;
			get_results(page+1);
		}else if(tab=='images'){
